...
def sample_distribution(
    dist: Optional[Distribution], rng: np.random.Generator
) -> float:
...
class Simulation:
    def __init__(self, scenario: Scenario, seed: Optional[int] = None):
        ...
        self.end_year = self.scenario.user_birth_year + int(
            self._get_life_expectancy(self.scenario.user_life_expectancy)
        )
        # Tracking investment values by investment ID
        self.investments: Dict[str, Decimal] = {
            inv.id.hex: inv.current_value for inv in self.scenario.investments or []
        }
        ...
        self.standard_deduction = (
            13850  # Example fixed, replace with user/state/year-dependent
        )
        self.inflation_rate = sp.Rational(
            str(0.02)
        )  # Assume 2%, or sample from assumption each year
    ...
    def run(self):
        ...
        while year < self.end_year:
            ...
            self.inflation_rate = sample_distribution(
                self.scenario.inflation_assumption, self.rng
            )
    ...
    def _run_income_events(self, year: int):
        income_events = [
            e
            for e in self.scenario.event_series or []
            if e.type == EventSeriesType.INCOME
        ]
        total_income = 0.0
        for event in income_events:
            start_year = (
                int(sample_distribution(event.start_year, self.rng))
                if event.start_year
                else self.current_year
            )
            duration = (
                int(sample_distribution(event.duration, self.rng))
                if event.duration
                else 1
            )
            if start_year <= year < start_year + duration:
                ...
                if event.annual_change:
                    ...
                    if (
                        isinstance(event.annual_change, FixedDistribution)
                        and event.inflation_adjusted
                    ):
                        # Fixed + inflation adjustment
                        amount = amount * (
                            (1 + annual_change + self.inflation_rate) ** years_active
                        )
                    else:
        ...
        if cash_investment_id:
            self.investments[cash_investment_id] = self.investments.get(
                cash_investment_id, Decimal(0)
    ...
    def _perform_rmd(self, year: int):
        ...
        total_pre_tax_value = sum(
            float(self.investments.get(inv.id.hex, Decimal(0)))
        ...
        for inv in pre_tax_investments:
            ...
            non_ret_inv = next(
                (
                    i
                    for i in (self.scenario.investments or [])
                    if i.investment_type_id == inv.investment_type_id
                    and i.account_tax_status == AccountTaxStatus.NON_RETIREMENT
                ),
                None,
            )
            if non_ret_inv:
                self.investments[non_ret_inv.id.hex] = self.investments.get(
                    non_ret_inv.id.hex, Decimal(0)
    ...
    def _update_investments(self, year: int):
        ...
        for inv in self.scenario.investments or []:
            ...
            inv_type = next(
                (
                    t
                    for t in (self.scenario.investment_types or [])
                    if t.id == inv.investment_type_id
                ),
                None,
            )
            if not inv_type:
            ...
            annual_return_rate = sample_distribution(
                inv_type.expected_annual_return, self.rng
            )
            ...
            annual_income = sample_distribution(
                inv_type.expected_annual_income, self.rng
            )
            income_value = (
                value * annual_income if inv_type.income_percent else annual_income
            )
    ...
    def _pay_expenses(self, year: int, discretionary: bool = False):
        ...
        expense_events = [
            e
            for e in (self.scenario.event_series or [])
            if e.type == EventSeriesType.EXPENSE and e.is_discretionary == discretionary
        ]
        total_expense = 0.0
        ...
        print(
            f"Year {year} paid {'discretionary' if discretionary else 'non-discretionary'} expenses: {total_expense:.2f}"
        )
