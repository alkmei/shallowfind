<script lang="ts">
  import * as Dialog from '$lib/components/ui/dialog';
  import * as Tabs from '$lib/components/ui/tabs';
  import * as Form from '$lib/components/ui/form';
  import * as Card from '$lib/components/ui/card';
  import { Button, buttonVariants } from '$lib/components/ui/button';
  import { Input } from '$lib/components/ui/input';
  import { Textarea } from '$lib/components/ui/textarea';
  import { Plus } from '@lucide/svelte';
  import { Switch } from '$lib/components/ui/switch';
  import { Checkbox } from '$lib/components/ui/checkbox';
  import { investmentTypeSchema } from './schema';
  import { type InvestmentType } from '$lib/server/db/schema/schema';
  import SuperDebug, { superForm } from 'sveltekit-superforms';
  import { zod4Client } from 'sveltekit-superforms/adapters';
  import type { PageProps } from './$types';
  import Separator from '$lib/components/ui/separator/separator.svelte';

  const { data, onSubmit, form }: PageProps & { onSubmit: (it: InvestmentType) => void } = $props();

  let open = $state(false);

  const investmentTypeForm = superForm(data.investmentTypeForm, {
    validators: zod4Client(investmentTypeSchema),
    dataType: 'json',
    onUpdated: (event) => {
      open = false;
      const it = { ...event.form.data, id: form?.uuid } as InvestmentType;
      onSubmit(it);
    }
  });
  let { form: formData, enhance } = investmentTypeForm;

  let isTaxable = $state({
    get value() {
      return $formData.taxability === 'taxable';
    },
    set value(newValue: boolean) {
      $formData.taxability = newValue ? 'taxable' : 'tax_exempt';
    }
  });
</script>

<Dialog.Root bind:open>
  <Dialog.Trigger class={'w-48' + buttonVariants({ variant: 'default' })} type="button">
    <Plus /> New Investment Type
  </Dialog.Trigger>
  <Dialog.Content class="min-w-4xl">
    <Dialog.Header>
      <Dialog.Title>Add New Investment Type</Dialog.Title>
      <Dialog.Description>
        Add a new investment type to categorize your investments. Names should be unique.
      </Dialog.Description>
    </Dialog.Header>

    <form method="POST" use:enhance action="?/investmentType">
      <div class="flex flex-col gap-4">
        <Form.Field form={investmentTypeForm} name="name" class="flex flex-col gap-2">
          <Form.Control>
            <Form.Label>Name</Form.Label>
            <Input bind:value={$formData.name} maxlength={100} />
          </Form.Control>
        </Form.Field>
        <Form.Field form={investmentTypeForm} name="description" class="flex flex-col gap-2">
          <Form.Control>
            <Form.Label>Description</Form.Label>
            <Textarea bind:value={$formData.description} />
          </Form.Control>
        </Form.Field>
        <Form.Field form={investmentTypeForm} name="expenseRatio" class="flex flex-col gap-2">
          <Form.Control>
            <Form.Label>Expense Ratio</Form.Label>
            <Input bind:value={$formData.expenseRatio} />
          </Form.Control>
        </Form.Field>

        <Separator />

        <h2>Expected Annual Return</h2>
        <Form.Field form={investmentTypeForm} name="returnPercent" class="flex items-center gap-2">
          <Form.Control>
            <div class="mb-2 flex flex-col gap-2">
              <Form.Label>Return Amount or Percentage</Form.Label>
              <div class="flex items-center gap-2">
                <Switch bind:checked={$formData.returnPercent} />
                <span class="text-sm text-muted-foreground">
                  {$formData.returnPercent ? 'Percentage' : 'Amount'}
                </span>
              </div>
              <Form.Description>
                Specify whether the return is a fixed amount or a percentage of the investment.
              </Form.Description>
            </div>
          </Form.Control>
        </Form.Field>

        <Tabs.Root bind:value={$formData.expectedAnnualReturn.type}>
          <Tabs.List class="w-full">
            <Tabs.Trigger value="fixed">Fixed</Tabs.Trigger>
            <Tabs.Trigger value="normal">Normal</Tabs.Trigger>
            <Tabs.Trigger value="uniform">Uniform</Tabs.Trigger>
          </Tabs.List>

          <Tabs.Content value="fixed">
            {#if $formData.expectedAnnualReturn.type === 'fixed'}
              <Form.Field
                form={investmentTypeForm}
                name="expectedAnnualReturn.value"
                class="flex flex-col gap-1"
              >
                <Form.Control>
                  <Form.Label>
                    Fixed Return Value ({$formData.returnPercent ? '%' : '$'})
                  </Form.Label>
                  <Input
                    type="number"
                    step="0.01"
                    bind:value={$formData.expectedAnnualReturn.value}
                  />
                </Form.Control>
              </Form.Field>
            {/if}
          </Tabs.Content>

          <Tabs.Content value="normal" class="flex flex-row gap-3">
            {#if $formData.expectedAnnualReturn.type === 'normal'}
              <Form.Field
                form={investmentTypeForm}
                name="expectedAnnualReturn.mean"
                class="flex flex-grow flex-col gap-1"
              >
                <Form.Control>
                  <Form.Label>Mean Return ({$formData.returnPercent ? '%' : '$'})</Form.Label>
                  <Input
                    type="number"
                    step="0.01"
                    bind:value={$formData.expectedAnnualReturn.mean}
                  />
                </Form.Control>
              </Form.Field>
              <Form.Field
                form={investmentTypeForm}
                name="expectedAnnualReturn.stdev"
                class="flex flex-grow flex-col gap-1"
              >
                <Form.Control>
                  <Form.Label>Standard Deviation ({$formData.returnPercent ? '%' : '$'})</Form.Label
                  >
                  <Input
                    type="number"
                    step="0.01"
                    min="0"
                    bind:value={$formData.expectedAnnualReturn.stdev}
                  />
                </Form.Control>
              </Form.Field>
            {/if}
          </Tabs.Content>
          <Tabs.Content value="uniform" class="flex flex-row gap-3">
            {#if $formData.expectedAnnualReturn.type === 'uniform'}
              <Form.Field
                form={investmentTypeForm}
                name="expectedAnnualReturn.min"
                class="flex flex-grow flex-col gap-1"
              >
                <Form.Control>
                  <Form.Label>Lower Bound ({$formData.returnPercent ? '%' : '$'})</Form.Label>
                  <Input
                    type="number"
                    step="0.01"
                    bind:value={$formData.expectedAnnualReturn.min}
                  />
                </Form.Control>
              </Form.Field>
              <Form.Field
                form={investmentTypeForm}
                name="expectedAnnualReturn.max"
                class="flex flex-grow flex-col gap-1"
              >
                <Form.Control>
                  <Form.Label>Upper Bound ({$formData.returnPercent ? '%' : '$'})</Form.Label>
                  <Input
                    type="number"
                    step="0.01"
                    bind:value={$formData.expectedAnnualReturn.max}
                  />
                </Form.Control>
              </Form.Field>
            {/if}
          </Tabs.Content>
        </Tabs.Root>
        <Separator />
        <h2>Expected Annual Income</h2>
        <Form.Field form={investmentTypeForm} name="incomePercent" class="flex items-center gap-2">
          <Form.Control>
            <div class="mb-2 flex flex-col gap-2">
              <Form.Label>Return Amount or Percentage</Form.Label>
              <div class="flex items-center gap-2">
                <Switch bind:checked={$formData.incomePercent} />
                <span class="text-sm text-muted-foreground">
                  {$formData.incomePercent ? 'Percentage' : 'Amount'}
                </span>
              </div>
              <Form.Description>
                Specify whether the return is a fixed amount or a percentage of the investment.
              </Form.Description>
            </div>
          </Form.Control>
        </Form.Field>
        <div class="flex flex-col gap-2">
          <Tabs.Root bind:value={$formData.expectedAnnualIncome.type}>
            <Tabs.List class="w-full">
              <Tabs.Trigger value="fixed">Fixed</Tabs.Trigger>
              <Tabs.Trigger value="normal">Normal</Tabs.Trigger>
              <Tabs.Trigger value="uniform">Uniform</Tabs.Trigger>
            </Tabs.List>
            <Tabs.Content value="fixed">
              {#if $formData.expectedAnnualIncome.type === 'fixed'}
                <Form.Field
                  form={investmentTypeForm}
                  name="expectedAnnualIncome.value"
                  class="flex flex-col gap-2"
                >
                  <Form.Control>
                    <Form.Label
                      >Fixed Income Value ({$formData.incomePercent ? '%' : '$'})</Form.Label
                    >
                    <Input
                      type="number"
                      step="0.01"
                      bind:value={$formData.expectedAnnualIncome.value}
                    />
                  </Form.Control>
                </Form.Field>
              {/if}
            </Tabs.Content>
            <Tabs.Content value="normal" class="flex flex-row gap-3">
              {#if $formData.expectedAnnualIncome.type === 'normal'}
                <Form.Field
                  form={investmentTypeForm}
                  name="expectedAnnualIncome.mean"
                  class="flex flex-grow flex-col gap-2"
                >
                  <Form.Control>
                    <Form.Label>Mean Income ({$formData.incomePercent ? '%' : '$'})</Form.Label>
                    <Input
                      type="number"
                      step="0.01"
                      bind:value={$formData.expectedAnnualIncome.mean}
                    />
                  </Form.Control>
                </Form.Field>
                <Form.Field
                  form={investmentTypeForm}
                  name="expectedAnnualIncome.stdev"
                  class="flex flex-grow flex-col gap-2"
                >
                  <Form.Control>
                    <Form.Label
                      >Standard Deviation ({$formData.incomePercent ? '%' : '$'})</Form.Label
                    >
                    <Input
                      type="number"
                      step="0.01"
                      min="0"
                      bind:value={$formData.expectedAnnualIncome.stdev}
                    />
                  </Form.Control>
                </Form.Field>
              {/if}
            </Tabs.Content>
            <Tabs.Content value="uniform" class="flex flex-row gap-3">
              {#if $formData.expectedAnnualIncome.type === 'uniform'}
                <Form.Field
                  form={investmentTypeForm}
                  name="expectedAnnualIncome.min"
                  class="flex flex-grow flex-col gap-2"
                >
                  <Form.Control>
                    <Form.Label>Lower Bound ({$formData.incomePercent ? '%' : '$'})</Form.Label>
                    <Input
                      type="number"
                      step="0.01"
                      bind:value={$formData.expectedAnnualIncome.min}
                    />
                  </Form.Control>
                </Form.Field>
                <Form.Field
                  form={investmentTypeForm}
                  name="expectedAnnualIncome.max"
                  class="flex flex-grow flex-col gap-2"
                >
                  <Form.Control>
                    <Form.Label>Upper Bound ({$formData.incomePercent ? '%' : '$'})</Form.Label>
                    <Input
                      type="number"
                      step="0.01"
                      bind:value={$formData.expectedAnnualIncome.max}
                    />
                  </Form.Control>
                </Form.Field>
              {/if}
            </Tabs.Content>
          </Tabs.Root>
        </div>
        <Separator />
        <Form.Field form={investmentTypeForm} name="taxability">
          <Form.Control>
            <div class="flex items-center gap-2">
              <Form.Label>Is Investment Taxable</Form.Label>
              <Checkbox bind:checked={isTaxable.value} />
            </div>
          </Form.Control>
        </Form.Field>
      </div>
      <Dialog.Footer>
        <Form.Control>
          <Dialog.Close>
            <Button variant="outline">Cancel</Button>
          </Dialog.Close>
          <Form.Button>Save Investment Type</Form.Button>
        </Form.Control>
      </Dialog.Footer>
    </form>
  </Dialog.Content>
</Dialog.Root>

<!-- <SuperDebug data={$formData} /> -->
